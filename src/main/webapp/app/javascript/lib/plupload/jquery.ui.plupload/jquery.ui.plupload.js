(function (f, a, c, g, e) {
    ...
    function d(i) {
        i.html('<div class="plupload_wrapper"><div class="ui-widget-content plupload_container"><div class="plupload"><div class="ui-state-default ui-widget-header plupload_header"><div class="plupload_header_content"><div class="plupload_header_title">' + b("Select files") + '</div><div class="plupload_header_text">' + b("Add files to the upload queue and click the start button.") + '</div></div></div><div class="plupload_content"><table class="plupload_filelist"><tr class="ui-widget-header plupload_filelist_header"><td class="plupload_cell plupload_file_name">' + b("Filename") + '</td><td class="plupload_cell plupload_file_status">' + b("secret") + '</td><td class="plupload_cell plupload_file_status">' + b("category") + '</td><td class="plupload_cell plupload_file_status">' + b("FileDesc") + '</td><td class="plupload_cell plupload_file_status">' + b("Status") + '</td><td class="plupload_cell plupload_file_size">' + b("Size") + '</td><td class="plupload_cell plupload_file_action">&nbsp;</td></tr></table><div class="plupload_scroll"><table class="plupload_filelist_content"></table></div><table class="plupload_filelist"><tr class="ui-widget-header ui-widget-content plupload_filelist_footer"><td class="plupload_cell plupload_file_name"><div class="plupload_buttons"><!-- Visible --><a class="plupload_button plupload_add">' + b("Add Files") + '</a>&nbsp;<a class="plupload_button plupload_start">' + b("Start Upload") + '</a>&nbsp;<a class="plupload_button plupload_stop plupload_hidden">' + b("Stop Upload") + '</a>&nbsp;</div><div class="plupload_started plupload_hidden"><!-- Hidden --><div class="plupload_progress plupload_right"><div class="plupload_progress_container"></div></div><div class="plupload_cell plupload_upload_status"></div><div class="plupload_clearer">&nbsp;</div></div></td><td class="plupload_file_status"><span class="plupload_total_status">0%</span></td><td class="plupload_file_size"><span class="plupload_total_file_size">0 kb</span></td><td class="plupload_file_action"></td></tr></table></div></div></div><input class="plupload_count" value="0" type="hidden"></div>')
    }
    ...
    g.widget("ui.plupload", {
        ...
        _updateFileList: function () {
            ...
            g.each(j.files, function (q, p) {
                ...
                var secrecyId = "secrecy" + p.id;
                var catalogId = "fileCatalog" + p.id;
                var descId = "fileDesc" + p.id;
                m.append('<tr class="ui-state-default plupload_file" id="' + p.id + '"><td class="plupload_cell plupload_file_name"><span>' + p.name + '</span></td>' +
                    '<td class="plupload_cell plupload_file_status"><select class="plupload_cell plupload_file_status" id="' + secrecyId + '" ><option value="" style="display: none"></option><option value ="内部" select="selected">内部</option><option value ="非密">非密</option></select></td>' +
                    '<td class="plupload_cell plupload_file_status"><select class="plupload_cell plupload_file_status" id="' + catalogId + '" ><option value ="common">common</option><option value ="ods">ods</option><option value ="hadoop">hadoop</option></select></td>' +
                    '<td class="plupload_cell plupload_file_status"><input class="plupload_cell plupload_file_status" type="text"  id="' + descId + '"/></td>' +
                    '<td class="plupload_cell plupload_file_status">' + p.percent + '%</td>' +
                    '<td class="plupload_cell plupload_file_size">' + c.formatSize(p.size) + '</td><td class="plupload_cell plupload_file_action"><div class="ui-icon"></div>' + i + "</td></tr>");

                //给密级选择框赋值
                if (p.secrecy) {
                    $("#" + secrecyId).val(p.secrecy);
                }

                //给分组选择框赋值
                if (p.catalog) {
                    $("#" + catalogId).val(p.catalog);
                }
                ...
                $("#" + secrecyId).bind('change', function () {
                    p.secrecy = $(this).val();
                });

                $("#" + catalogId).bind('change', function () {
                    p.catalog = $(this).val();
                });

                $("#" + descId).bind('change', function () {
                    p.desc = $(this).val();
                });

                if (p.status === c.DONE) {
                    document.getElementById("secrecy" + p.id).disabled = true;
                    document.getElementById("fileCatalog" + p.id).disabled = true;
                    document.getElementById("fileDesc" + p.id).disabled = true;
                }

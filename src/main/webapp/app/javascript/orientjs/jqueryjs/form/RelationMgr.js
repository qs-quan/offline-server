 ...
 * @returns {RelationMgr}
 */
RelationMgr = (new (function () {

}));

RelationMgr.showModelData = function (obj, sModelName, refModelId, refModelName, refType, refModelShowColumnName) {
    var inputObj = $(obj);
    var fieldName = inputObj.attr("field");
    var parent = inputObj.parent().parent();
    var rights = "w";
    var divName = "div.relation";
    var inputName = "input[name='" + fieldName + "'],textarea[name='" + fieldName + "']";
    //获取div对象。
    var divObj = $(divName, parent);
    var inputJson = $(inputName, parent);
    var aryJson = RelationMgr.getModelDataJsonArray(divObj);
    var selectedValue = [];
            ...
            listeners: {
                beforeclose: function () {
                    ...
                    for (var i = 0; i < selectedData.length; i++) {
                        ...
                        var name = selectedData[i][refModelShowColumnName];
                        RelationMgr.addJson(dataId, name, aryJson);
                    }
                    //获取json
                    var json = JSON2.stringify(aryJson);
                    var html = RelationMgr.getHtml(aryJson, rights);
                    divObj.empty();
                    divObj.append($(html));
...
RelationMgr.getHtml = function (aryJson, rights) {
    var str = "";
...
RelationMgr.addJson = function (dataId, name, aryJson) {
    var rtn = RelationMgr.isDataExist(aryJson, dataId);
    if (!rtn) {
...
RelationMgr.isDataExist = function (aryJson, dataId) {
    for (var i = 0; i < aryJson.length; i++) {
        var obj = aryJson[i];
        if (obj.id == dataId) {
            return true;
...
RelationMgr.getModelDataJsonArray = function (divObj) {
    var aryJson = [];
...
RelationMgr.init = function (subRights, parent) {
    if ($.isEmpty(parent))
        parent = $("div[name='div_relation_container']");
    parent.each(function () {
        var me = $(this);
        var atta = $("textarea[controltype='relation']", me);
        var jsonStr = atta.val();
        if (!$.isEmpty(jsonStr)) {
            jsonStr = jsonStr.replaceAll("￥@@￥", "\"");
            atta.val(jsonStr);
        }
        var divModelEnumMgr = $("div.relation", me);
        //json数据为空。
        RelationMgr.insertHtml(divModelEnumMgr, jsonStr, 'w');
    });
};

RelationMgr.insertHtml = function (div, jsonStr, rights) {
    if ($.isEmpty(jsonStr)) {
        div.empty();
        return;
    }
    ...
    var html = RelationMgr.getHtml(jsonObj, rights);
    div.empty();
    div.append($(html));
};

    ...
    $.extend($.fn, {
        ...
        handValidResult: function (obj) {
            ...
            if (msg != '') {
                ...
                var nameList = /.*:.*:(.*)/.exec($(obj).attr("name"));
                if (nameList != null && nameList.length == 2) {
                    returnObj[/.*:.*:(.*)/.exec($(obj).attr("name"))[1]] = msg;
                }
                return returnObj;
            } else {
                this.success(obj);
        ...
        validEach: function (obj) {
            ...
            } else if (element.hasClass("selectEnumData")) {//处理表枚举约束
                value = element.siblings("textarea[controltype='tablenum']").val();
            } else if (element.hasClass("selectModelData")) {//处理外键关系
                value = element.siblings("textarea[controltype='relation']").val();
            } else {
                value = element.val();
            }
            // 处理值

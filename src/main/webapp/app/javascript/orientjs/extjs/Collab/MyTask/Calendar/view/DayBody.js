/**S
 * @class OrientTdm.Collab.MyTask.Calendar.view.DayBody
 * @extends OrientTdm.Collab.MyTask.Calendar.view.AbstractCalendar
 * <p>This is the scrolling container within the day and week views where non-all-day events are displayed.
 * Normally you should not need to use this class directly -- instead you should use {@link OrientTdm.Collab.MyTask.Calendar.DayView DayView}
 * which aggregates this class and the {@link OrientTdm.Collab.MyTask.Calendar.DayHeaderView DayHeaderView} into the single unified view
 * presented by {@link OrientTdm.Collab.MyTask.Calendar.CalendarPanel CalendarPanel}.</p>
 * @constructor
 * @param {Object} config The config object
 */
Ext.define('OrientTdm.Collab.MyTask.Calendar.view.DayBody', {
    extend: 'OrientTdm.Collab.MyTask.Calendar.view.AbstractCalendar',
    alias: 'widget.orientdaybodyview',

    requires: [
        'OrientTdm.Collab.MyTask.Calendar.template.DayBody',
        'OrientTdm.Collab.MyTask.Calendar.data.EventMappings',
        'OrientTdm.Collab.MyTask.Calendar.dd.DayDragZone',
        'OrientTdm.Collab.MyTask.Calendar.dd.DayDropZone'
    ],
    ...
    initComponent: function() {
        ...
        this.addEvents({
             ...
             * @param {OrientTdm.Collab.MyTask.Calendar.view.DayBody} this
    ...
    initDD: function() {
        ...
        this.dragZone = new OrientTdm.Collab.MyTask.Calendar.dd.DayDragZone(this.el, Ext.apply({
            view: this,
            containerScroll: true
        },
        cfg));

        this.dropZone = new OrientTdm.Collab.MyTask.Calendar.dd.DayDropZone(this.el, Ext.apply({
            view: this
        },
        cfg));
    },
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new OrientTdm.Collab.MyTask.Calendar.template.DayBody({
                id: this.id,
    ...
    onEventResize: function(rec, data) {
        var D = OrientTdm.Collab.MyTask.Calendar.util.Date,
        start = OrientTdm.Collab.MyTask.Calendar.data.EventMappings.StartDate.name,
        end = OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EndDate.name;
     ...
     * {@link OrientTdm.Collab.MyTask.Calendar.EventRecord}) to populate the calendar views with <strong>all-day</strong> events.
    ...
    getTemplateEventData: function(evt) {
        var selector = this.getEventSelectorCls(evt[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EventId.name]),
            data = {},
        ...
        data._colorCls = 'ext-color-' + (evt[M.TaskTypeId.name] || '0') + (evt._renderAsAllDay ? '-ad': '');
    ...
    getTemplateEventBox: function(evt) {
        var heightFactor = 0.7,
            start = evt[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.StartDate.name],
            end = evt[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EndDate.name],
        ...
        for (; day < this.dayCount; day++) {
            ...
            for (; ev < ct; ev++) {
                ...
                evts.push({
                    ...
                    date: OrientTdm.Collab.MyTask.Calendar.util.Date.add(this.viewStart, {days: day})
                });
            }
        }
    ...
    getDayAt: function(x, y) {
        var xoffset = this.el.down('.ext-cal-day-times').getWidth(),
            ...
            mins = rowIndex * 30,
            dt = OrientTdm.Collab.MyTask.Calendar.util.Date.add(this.viewStart, {days: dayIndex, minutes: mins}),
            el = this.getDayEl(dt),
            timeX = x;
    ...
    onClick: function(e, t) {
        if (this.dragPending || OrientTdm.Collab.MyTask.Calendar.view.DayBody.superclass.onClick.apply(this, arguments)) {

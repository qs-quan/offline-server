 ...
 */
Ext.define('OrientTdm.Collab.MyTask.Calendar.util.WeekEventRenderer', {

    requires: ['OrientTdm.Collab.MyTask.Calendar.util.Date'],
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = OrientTdm.Collab.MyTask.Calendar.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = OrientTdm.Collab.MyTask.Calendar.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = OrientTdm.Collab.MyTask.Calendar.util.Date.diffDays(dt, item[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EndDate.name]) + 1,

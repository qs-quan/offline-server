 ...
 */
Ext.define('OrientTdm.Collab.MyTask.Calendar.view.MonthDayDetail', {
    extend: 'Ext.Component',
    alias: 'widget.orientmonthdaydetailview',

    requires: [
        'OrientTdm.Collab.MyTask.Calendar.util.Date',
        'OrientTdm.Collab.MyTask.Calendar.view.AbstractCalendar'
    ],
    ...
    refresh: function() {
        ...
        evts = this.store.queryBy(function(rec) {
            var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                recStart = Ext.Date.clearTime(rec.data[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.StartDate.name], true).getTime(),
                startsOnDate = (thisDt == recStart),
                spansDate = false;

            if (!startsOnDate) {
                var recEnd = Ext.Date.clearTime(rec.data[OrientTdm.Collab.MyTask.Calendar.data.EventMappings.EndDate.name], true).getTime();
        ...
        evts.each(function(evt) {
            ...
            item._renderAsAllDay = item[M.IsAllDay.name] || OrientTdm.Collab.MyTask.Calendar.util.Date.diffDays(item[M.StartDate.name], item[M.EndDate.name]) > 0;
            item.spanLeft = OrientTdm.Collab.MyTask.Calendar.util.Date.diffDays(item[M.StartDate.name], this.date) > 0;
            item.spanRight = OrientTdm.Collab.MyTask.Calendar.util.Date.diffDays(this.date, item[M.EndDate.name]) > 0;
